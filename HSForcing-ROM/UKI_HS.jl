...
function visualize(rukiobj::UKIObj{Float64}, θ_ref::Array{Float64, 1}, file_name::String)
  ...
  parameter_names = rukiobj.unames
  for i_θ = 1:n_θ
...
function HS_RUKI(t_mean::Array{Float64,1}, t_cov::Array{Float64,2}, θ_bar::Array{Float64,1}, θθ_cov::Array{Float64,2}, 
  end_day::Int64, spinup_day::Int64, N_data::Int64, α_reg::Float64, 
  N_iter::Int64 = 100)
  ...
  for i in 1:N_iter
    ...
    @info "At iter ", i, " params_i : ", rukiobj.θθ_cov[end]
    ...
    if i%10 == 0
      ...
      visualize(rukiobj, θ_ref, "UKI_Obj_Ite_"*string(i)*".pdf") 
    end
  end
  
  @info "θ is ", rukiobj.θ_bar[end], " θ_ref is ",  
...
N_ite = 20
α_reg = 1.0
rukiobj = HS_RUKI(t_mean, t_cov, θ0_bar, θθ0_cov,  end_day, spinup_day, N_data, α_reg, N_ite)
