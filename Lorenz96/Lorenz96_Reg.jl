...
for i = 1:3
  ...
  ax.hist(data_ref[:,1:K][:], bins = nbin, color="black", density = true, histtype = "step", label="Truth")
  ax.hist(data_0[:,1:K][:], bins = nbin, color="C5", density = true, histtype = "step", label="Prior")
  data = Run_Lorenz96(phys_params, Q0, exkiobjs[1].θ_bar[end], Φ)
  ax.hist(data[:,1:K][:], bins = nbin, color="C1", density = true, histtype = "step", label="0%")
  data = Run_Lorenz96(phys_params, Q0, exkiobjs[2].θ_bar[end], Φ)
  ax.hist(data[:,1:K][:], bins = nbin, color="C2", density = true, histtype = "step", label="1%")
  data = Run_Lorenz96(phys_params, Q0, exkiobjs[3].θ_bar[end], Φ)
  ax.hist(data[:,1:K][:], bins = nbin, color="C3", density = true, histtype = "step", label="5%")
  ...
  ax.plot(xx, fDA, c="C1", linestyle="--", marker="o", fillstyle="none", markevery=100,  label="0%")
  for i = 1:length(xx);  fDA[i] = Φ(xx[i], exkiobjs[2].θ_bar[end]); end
  ax.plot(xx, fDA, c="C2", linestyle="--", marker="o", fillstyle="none", markevery=100, label="1%")
  for i = 1:length(xx);  fDA[i] = Φ(xx[i], exkiobjs[3].θ_bar[end]); end
  ax.plot(xx, fDA, c="C3", linestyle="--", marker="o", fillstyle="none", markevery=100, label="5%")
