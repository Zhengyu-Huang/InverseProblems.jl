...
Δt = 0.5
@time gmkiobj = GMKI_Run(s_param, aug_forward, θ0_w, θ0_mean, θθ0_cov, aug_y, aug_Σ_η, Δt, update_freq, N_iter; unscented_transform="modified-2n+1")
@save "gmkiobj-seed$(seed).jld2" gmkiobj
...
grid_vor = Initial_ω0_KL(mesh, gmkiobj.θ_mean[N_iter][1,:], s_param.seq_pairs)   
plot_field(mesh, grid_vor,  color_lim, ax_vor[2]) 
ax_vor[2].set_title("Mode 1")

grid_vor = Initial_ω0_KL(mesh, gmkiobj.θ_mean[N_iter][2,:], s_param.seq_pairs)   
plot_field(mesh, grid_vor,  color_lim, ax_vor[3]) 
ax_vor[3].set_title("Mode 2")

grid_vor = Initial_ω0_KL(mesh, gmkiobj.θ_mean[N_iter][3,:], s_param.seq_pairs)   
...
for m = 1:N_modes
    for i = 1:N_iter
        ...
        grid_vor = Initial_ω0_KL(mesh, gmkiobj.θ_mean[i][m,:], s_param.seq_pairs)   
