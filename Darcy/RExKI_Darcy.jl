...
function Darcy_Test(darcy::Param_Darcy, N_θ::Int64, α_reg::Float64, N_ite::Int64 = 100, noise_level::Float64=-1.0)
    ...
    exkiobj = ExKI_Run(t_mean, t_cov, θ0_bar, θθ0_cov, darcy,  α_reg, N_ite)
...
function Plot_Params(exkiobj::ExKIObj, filename::String = "None")
    N_θ = 5 #first 2 components
    Nite = length(exkiobj.θ_bar)
    @info Nite
    θ_bar = exkiobj.θ_bar
...
function Plot_Ite_Error(exkiobj::ExKIObj, label::String, filename::String = "None")
    Nite = length(exkiobj.θ_bar)
    @info Nite
    ...
    for i = 1:N_ite
        ...
        errors[1, i] = norm(darcy.logκ_2d - compute_logκ_2d(darcy, exkiobj.θ_bar[i]))/norm(darcy.logκ_2d)
        errors[2, i] = 0.5*(exkiobj.g_bar[i] - exkiobj.g_t)'*(exkiobj.obs_cov\(exkiobj.g_bar[i] - exkiobj.g_t))
...
function Plot_Ite_Datamismatch(exkiobj::ExKIObj, label::String, filename::String = "None")
    Nite = length(exkiobj.θ_bar)
    @info Nite
...
plot_field(darcy, compute_logκ_2d(darcy, exkiobj.θ_bar[N_ite]), false, "Darcy-logk-32.pdf")


####################################################################################
Plot_Params(exkiobj, "darcy_error_32.pdf")
Plot_Ite_Error(exkiobj, "Nθ=32", "Darcy-Params.pdf")
Plot_Ite_Datamismatch(exkiobj, "Nθ=32", "Darcy-Data-Mismatch.pdf")
@info "finished"
