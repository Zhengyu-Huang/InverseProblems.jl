...
function GAUKI_Run(t_mean, t_cov, θ_bar, θθ_cov, Tobs::Float64 = 10.0, Tspinup::Float64 = 30.0, Δt::Float64 = 0.01)
    ...
    gaukiobj = GAUKIObj(parameter_names,
    θ_bar, 
    θθ_cov,
    t_mean, # observation
    t_cov,
    ...
    ϵt = 1.0e-8
    while gaukiobj.t <= 1.0 - ϵt
        # Note that the parameters are exp-transformed for use as input
        # to Cloudy
        params_i = deepcopy(gaukiobj.θ_bar[end])
        
        @info "t is  ", gaukiobj.t, " params_i : ", params_i
        
        update_ensemble!(gaukiobj, ens_func) 

    end
    
    @info "θ is ", gaukiobj.θ_bar[end], " θ_ref is ",  10.0, 28.0, 8.0/3.0
...
gaukiobj = GAUKI_Run(t_mean, t_cov, θ0_bar, θθ0_cov,  Tobs, Tspinup, Δt)

N_ite = length(gaukiobj.θ_bar) - 1

ites = Array(LinRange(1, N_ite+1, N_ite+1))
θ_bar = gaukiobj.θ_bar
θθ_cov = gaukiobj.θθ_cov
...
grid("on")
tight_layout()
savefig("Lorenz_inverse-GAUKI.pdf")
close("all")