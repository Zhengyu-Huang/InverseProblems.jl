...
if mission == "adjoint_plt"
    ...
    θ0_bar = [5.0;]          # mean 
    θθ0_cov = reshape([1.0^2], (1,1))        # standard deviation
    ...
    for i = 1:N_ite+1
        θθ_cov_arr[1,i] = sqrt(θθ_cov[i][1,1])
    end
    
    @info "final result is ", θ_bar[end],  θθ_cov[end]
    errorbar(ites, θ_bar_arr[1,:], yerr=3.0*θθ_cov_arr[1,:], fmt="--o",fillstyle="none", label=L"r")
