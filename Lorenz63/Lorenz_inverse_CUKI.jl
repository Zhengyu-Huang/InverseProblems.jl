...
function CUKI_Run(algorithm, t_mean, t_cov, θ_bar, θθ_cov, N_iter::Int64 = 100, Tobs::Float64 = 10.0, Tspinup::Float64 = 30.0, Δt::Float64 = 0.01)
    ...
    for i in 1:N_iter
        ...
        params_i = deepcopy(cukiobj.θ_bar[end])
        
        update_ensemble!(cukiobj, ens_func) 
        @info norm(params_i)
        
    end
    
    @info "θ is ", cukiobj.θ_bar[end], " θ_ref is ",  10.0, 28.0, 8.0/3.0
...
cukiobj = CUKI_Run(algorithm, t_mean, t_cov, θ0_bar, θθ0_cov, N_iter, Tobs, Tspinup, Δt)

N_ite = length(cukiobj.θ_bar) - 1

ites = Array(LinRange(1, N_ite+1, N_ite+1))
θ_bar = cukiobj.θ_bar
θθ_cov = cukiobj.θθ_cov
